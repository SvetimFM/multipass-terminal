...
function openCubicleManagement() {
  if (!currentAIOfficeProject) return;
...
function closeCubicleManagement() {
  document.getElementById('cubicle-management-modal').classList.add('hidden');
}

function populateCubicleManagementList() {
  if (!currentAIOfficeProject || !currentAIOfficeProject.aiOffice) return;
  
  const listContainer = document.getElementById('cubicle-management-list');
  listContainer.innerHTML = '';
  
  currentAIOfficeProject.aiOffice.cubicles.forEach((cubicle, idx) => {
    ...
    cubicleDiv.innerHTML = `
      ...
      <div class="flex gap-2">
        <button onclick="refreshCubicle('${currentAIOfficeProject.id}', ${idx})" 
                class="px-3 py-1 bg-blue-600 rounded text-sm" title="Refresh from GitHub">
          🔄 Refresh
        </button>
        <button onclick="pullFromMainCubicle('${currentAIOfficeProject.id}', ${idx})" 
                class="px-3 py-1 bg-green-600 rounded text-sm" title="Pull from main">
          ⬇️ Pull
        </button>
        <button onclick="resetCubicle('${currentAIOfficeProject.id}', ${idx})" 
                class="px-3 py-1 bg-yellow-600 rounded text-sm" title="Reset cubicle">
          ⚠️ Reset
        </button>
        <button onclick="deleteCubicleFromManagement('${currentAIOfficeProject.id}', ${idx})" 
...
async function refreshAllCubicles() {
  if (!currentAIOfficeProject || !currentAIOfficeProject.githubUrl) {
  ...
  try {
    const response = await fetch(`/api/projects/${currentAIOfficeProject.id}/ai-office/refresh-all`, {
      method: 'POST'
    });
...
async function pullFromMainAll() {
  if (!confirm('This will pull latest changes from main branch in all cubicles. Continue?')) {
    return;
  }
  
  try {
    const response = await fetch(`/api/projects/${currentAIOfficeProject.id}/ai-office/pull-all`, {
      method: 'POST'
    });
...
async function resetAllCubicles() {
  ...
  try {
    const response = await fetch(`/api/projects/${currentAIOfficeProject.id}/ai-office/reset-all`, {
      method: 'POST'
    });
...
async function refreshCubicle(projectId, cubicleIdx) {
  if (!currentAIOfficeProject.githubUrl) {
    alert('No GitHub URL configured for this project');
    return;
  }
...
async function pullFromMainCubicle(projectId, cubicleIdx) {
...
async function resetCubicle(projectId, cubicleIdx) {
...
async function deleteCubicleFromManagement(projectId, cubicleIdx) {
  ...
  try {
    ...
    if (response.ok) {
      // Reload projects and update the view
      await loadProjects();
      openAIOfficeGrid(projectId);
...
async function addMultipleCubicles() {
  ...
  try {
    ...
    if (added > 0) {
      ...
      // Reload projects and update views
      await loadProjects();
      openAIOfficeGrid(currentAIOfficeProject.id);
      closeCubicleManagement();
    }
